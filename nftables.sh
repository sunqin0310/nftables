    ...
    while true; do
        ...
        if [ -n "$port" ]; then
            ...
            if [[ "$port" == "22" ]]; then
                ...
                read -p "确认要关闭SSH访问吗? (输入 'yes' 确认，输入 q 退出): " confirm
...
show_firewall_ports() {
    ...
    local tcp_ports=$(nft_cmd list chain inet firewall input 2>/dev/null | grep "tcp dport" | grep "accept" | grep -oE "dport [0-9]+" | cut -d' ' -f2 | sort -n | uniq)
    ...
    if [ -z "$tcp_ports" ]; then
        ...
        for port in $tcp_ports; do
            # 获取端口对应的服务名称
            local service_name=$(grep "^$port/tcp" /etc/services 2>/dev/null | awk '{print $1}' | head -1)
            if [ -n "$service_name" ]; then
                print_message "  ✓ 端口 $port ($service_name): 开放" "$GREEN"
            else
                print_message "  ✓ 端口 $port: 开放" "$GREEN"
            fi
        done
    fi
    echo
    ...
    local udp_ports=$(nft_cmd list chain inet firewall input 2>/dev/null | grep "udp dport" | grep "accept" | grep -oE "dport [0-9]+" | cut -d' ' -f2 | sort -n | uniq)
    ...
    if [ -z "$udp_ports" ]; then
        ...
        for port in $udp_ports; do
            # 获取端口对应的服务名称
            local service_name=$(grep "^$port/udp" /etc/services 2>/dev/null | awk '{print $1}' | head -1)
            if [ -n "$service_name" ]; then
                print_message "  ✓ 端口 $port ($service_name): 开放" "$GREEN"
            else
                print_message "  ✓ 端口 $port: 开放" "$GREEN"
